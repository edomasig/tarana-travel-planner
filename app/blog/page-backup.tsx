...
async function getBlogPosts(page: number = 1, sortBy: string = 'newest', limit: number = 6) {
  try {
    const orderBy = (() => {
      switch (sortBy) {
        case 'oldest':
    ...
    const response = await fetch(`/api/blog/posts?page=${page}&limit=${limit}&sortBy=${sortBy}`, {
      cache: 'no-store'
    })
    ...
    return { posts: [], pagination: { page: 1, pages: 1, total: 0, limit: 6 } }
  }
}
...
export default function BlogPage() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      setLoading(true)
      const data = await getBlogPosts(currentPage, sortBy, 6)
      setBlogData(data)
      setLoading(false)
    }

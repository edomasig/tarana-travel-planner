...
export function GalaGPTLogoHorizontal({ 
  ...
  // Create unique IDs to prevent conflicts when multiple logos are on page
  const logoId = `logoGradient-${size}`
  const textId = `textGradient-${size}`
  ...
  return (
    <div className={`flex items-center ${className}`}>
      ...
      >
        ...
        <circle cx="30" cy="30" r="22" fill={`url(#${logoId})`} opacity="0.1"/>
        
        {/* Island/Mountain silhouette */}
        <path d="M8 35 C12 25, 18 22, 25 28 C32 20, 38 25, 45 30 C48 32, 50 35, 52 35 Z" fill={`url(#${logoId})`} opacity="0.8"/>
        ...
        <path d="M35 35 L35 25 M35 25 C32 23, 30 22, 28 24 M35 25 C38 23, 40 22, 42 24 M35 25 C33 20, 31 19, 29 21 M35 25 C37 20, 39 19, 41 21" 
              stroke={`url(#${logoId})`} strokeWidth="2" fill="none" strokeLinecap="round"/>
        ...
        {/* Text: GalaGPT */}
        <text x="70" y="25" fontFamily="system-ui, -apple-system, sans-serif" fontSize="20" fontWeight="700" fill={`url(#${textId})`}>
          Gala<tspan fill={`url(#${logoId})`}>GPT</tspan>
        </text>
        ...
        <g transform="translate(170, 15)">
          ...
          <circle cx="3" cy="8" r="1" fill="#FCD116"/>
